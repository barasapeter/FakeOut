...
def predict_fake_news(news_article: pd.DataFrame):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        df[['title', 'text', 'subject']], df['label'], test_size=0.2)
    ...
    if probability >= high_threshold:
        statement = f"My analysis suggests this news article appears to be entirely fake. The prediction accuracy is {accuracy * 100:.2f}%, so approach with significant skepticism."
    elif probability >= moderate_threshold:
        statement = f"This article seems to mix both real and fake elements based on my assessment. The prediction accuracy is {accuracy * 100:.2f}%, so critical evaluation is important."
    else:
        statement = f"This news article appears to be entirely true based on my prediction. The accuracy is {accuracy * 100:.2f}%. However, as with all news, further verification is essential."
    return statement
